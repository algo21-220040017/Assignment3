...
# 上证指数
result = w.wsd("000001.SH", "pre_close,open,close,high3,low3", start_date, today_date,
               "PriceAdj=F")
...
# 深证成值
result = w.wsd("399001.SZ", "pre_close,open,close,high3,low3", start_date, today_date, "PriceAdj=F")
szcz = pd.DataFrame(result.Data, columns=result.Times).transpose()
szcz.set_axis(['前收盘价','开盘价','收盘价','最高价','最低价'], axis='columns', inplace=True)
szcz.index.name = '日期'

# 中证500
result = w.wsd("000905.SH", "pre_close,open,close,high3,low3", start_date,
